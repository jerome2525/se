jQuery(document).ready(function($) {      
    ...
    if(output.extra_child && output.extra_child_second && output.extra_child_third) {
        ...
        $('#input_2_7').keyup(function() {
            computeInput();
        });
    }

    $('#input_2_3,#input_2_4').change(function() {
        fixedPriceCompute();
        computeInput();
    });

    if(output.fixed_price) {
        $('#input_2_4').addClass('disabled-input');
        setTimeout( function(){
    ...
    function fixedPriceCompute() {
        if(output.fixed_price) {
            var eDate = $('#input_2_3').val().split("/");
            var year = eDate[2];
            var month = eDate[1];
            var day = parseInt(eDate[0]) + 1;
            var end_date = day + '/'+ month + '/'+ year;
            console.log(end_date);
            $('#input_2_4').val(end_date);
        }
    }

    function computeInput() {
        var startDay = $('#input_2_3').val();
        var endDay = $('#input_2_4').val();
        var extraChild = $('#input_2_7').val();
        if( startDay.length && endDay.length ) {
            var dd = parseInt(endDay) - parseInt(startDay);

            if( startDay > endDay ) {
                $('.single_add_to_cart_button').addClass('disabled');  
                $('#input_2_3').addClass('err'); 
                $('#input_2_4').addClass('err');
            }
            else {
                $('.single_add_to_cart_button').removeClass('disabled');   
                $('#input_2_3').removeClass('err'); 
                $('#input_2_4').removeClass('err');  
            }
